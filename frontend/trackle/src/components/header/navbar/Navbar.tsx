        ...
        <ul className='flex flex-col lg:flex-row flex-nowrap items-center gap-2 '>
            {
                navItems.map((item, index) => (
                    <li key={index} className='hover:scale-105 box-border border-2 px-2 rounded-xl hover:bg-[var(--color-secondary)] transition-all duration-150 w-14 md:w-16 lg:w-18'>
                        <Link to={item.path} 
            ...
            {role &&<li className='hover:scale-105 box-border border-2 px-2 rounded-xl hover:bg-[var(--color-surface)] transition-all duration-150 w-14 md:w-15 lg:w-16'> <Link to={role === 'teacher' ? '/admin':`/user/${name}`} activeProps={{ className : 'text-green-500'}} onClick={ () => setOpen?.(false) }>{role == 'teacher' ? 'Admin': 'User' }</Link>
            </li>}
            <li>
