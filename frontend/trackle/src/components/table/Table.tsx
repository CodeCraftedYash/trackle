...
const Table: React.FC = () => {
  ...
  const tableRows = gettableRowsData.map((item) => {
    ...
    return (<tr key={item._id} className='w-full hover:bg-[var(--color-accent)] transition-all duration-300 ease-in-out'>
      ...
      <td className='p-2 min-w-[12rem] max-w-[13rem] text-left font-bold text-[var(--color-text)] truncate border-b-2 border-[var(--color-surface)] text-wrap' style={{ fontSize: "var(--font-size-base)" }}>{item.homework?.[today] ?? "No homework"}</td>
    </tr>)
  })
  return (
